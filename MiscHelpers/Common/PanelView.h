...
public:
	CPanelWidgetTmpl(T* pTree, QWidget *parent = 0) : B(parent)
	{
		...
		this->connect(m_pTreeList, SIGNAL(customContextMenuRequested( const QPoint& )), this, SLOT(OnMenu(const QPoint &)));
		m_pMainLayout->addWidget(m_pTreeList);
		m_pTreeList->setMinimumHeight(50);
		this->AddPanelItemsToMenu();

		m_pLastAction = this->m_pMenu->actions()[0];
	}

	virtual QMenu*				GetMenu()	{ return this->m_pMenu; }
	virtual void				AddAction(QAction* pAction) { this->m_pMenu->insertAction(m_pLastAction, pAction); }
